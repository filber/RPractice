...
add.indicator(strategy = q.strategy, name = "ROC",
              arguments = list(x = quote(Cl(Data)),n = ROC.INDICATOR.ROC.N),
              label = "ROC")
...
add.signal(q.strategy, name = "sigThreshold",
           arguments = list(column = "Close.ROC",threshold = ROC.SIGNAL.ROC.GT.HIGH,relationship = "gt"),
           label = "ROC.GT.HIGH")

#信号4:ROC小于支撑线
add.signal(q.strategy, name = "sigThreshold",
           arguments = list(column = "Close.ROC",threshold = ROC.SIGNAL.ROC.LT.LOW,relationship = "lt"),
           label = "ROC.LT.LOW")
...
add.rule(q.strategy, name = "ruleSignal",label = "ROC.BUY.GOLDEN.CROSS",
         arguments = list(
           ...
           orderqty = ORDER.QTY.BUY, ordertype = "market",osFUN="osMaxPos",orderside = "long", pricemethod = "market",portfolio=q.strategy),
         type = "enter")
...
add.rule(q.strategy, name = "ruleSignal",label = "ROC.BUY.DEATH.CROSS",
         arguments = list(
           ...
           orderqty = ORDER.QTY.SELL, ordertype = "market", orderside = "long",osFUN="osMaxPos", pricemethod = "market",portfolio=q.strategy),
         type = "exit")
...
add.distribution(strategy = q.strategy,paramset.label = 'ROC',
                 ...
                 variable = list(n = ROC.DIS.INDICATOR.ROC.N),
...
add.distribution(strategy = q.strategy,paramset.label = 'ROC',
                 ...
                 variable = list(threshold = ROC.DIS.SIGNAL.ROC.GT.HIGH),
                 label = 'ROC.DIS.SIGNAL.ROC.GT.HIGH')

#参数4:信号ROC下限值
add.distribution(strategy = q.strategy,paramset.label = 'ROC',
                 component.type = 'signal',component.label = 'ROC.LT.LOW',
                 variable = list(threshold = ROC.DIS.SIGNAL.ROC.LT.LOW),
